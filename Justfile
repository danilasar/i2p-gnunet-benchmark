...
test:
    docker run --rm --privileged -v $(pwd):/practice -w /practice coursework-overlay:latest go test ./testbed/... -v -run Smoke -timeout 5m

# Запустить только GNUnet smoke
test-gnunet:
    docker run --rm --privileged -v $(pwd):/practice -w /practice coursework-overlay:latest go test ./testbed/... -v -run TestGnunetSmoke -timeout 2m
